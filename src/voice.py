...
class Voice():
    ...
    async def voicEventTraitment(self, member, before, after):
        ...
        if (member.voice is not None and
            ...
            if str(member.id) not in data:
                data[str(member.id)] = [member.name + "'s chanel", 0, None]
    ...
    async def voiceCommande(self, message):
        if message.content == "v?voice lock":
            return await self.lock(message.author)
        elif message.content == "v?voice unlock":
            return await self.unlock(message.author)
        elif message.content == "v?voice claim":
        ...
        if chan is not None:
            ...
            if (chan.category_id == 797606253479329812 and
                ...
                if (str(member.id) in data and data[str(member.id)][1] == 0):
                    ...
                    self.save(data)
                    reponse = "{}\nTu possède maintenant".format(
                        member.mention)
                    return reponse + " le channel {}".format(
                        chan.name)
                return "{}\nTu possède déjà le channel".format(member.mention)
            return "{}\nTu n'es pas dans un channel créé par vcorp bot".format(
                member.mention)
